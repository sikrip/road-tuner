    ...
    static JFreeChart chartForRuns(List<DynoRunGuiEntry> runs) {
        ...
        for (DynoRunGuiEntry run : runs) {
            ...
            if (run.isActive()) {
               ...
               DynoSimulationEntry maxPower = result.maxPower();
               DynoSimulationEntry maxTorque = result.maxTorque();

               DynoSimulationEntry minPower = result.minPower();
               DynoSimulationEntry minTorque = result.minTorque();
                ...
                xySplineRenderer.setSeriesPaint(1, run.getColor());
                ...
                dataset.addSeries(run.getInfo().getName() + " power", result.powerDataset());
                dataset.addSeries(run.getInfo().getName() + " torque", result.torqueDataset());

                plot.setDataset(dataSetIdx, dataset);
                plot.setRenderer(dataSetIdx, xySplineRenderer);

                String annotationText = getPlotAnnotationText(run.getInfo().getName(), maxPower, maxTorque);
                ...
                annotation.setPaint(run.getColor());
